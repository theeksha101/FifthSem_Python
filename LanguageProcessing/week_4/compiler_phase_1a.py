    ...
    for token in tokens:
        for class_number in range(1, len_emot + 1):
            if token is None:
                ...
                output_file.write(f"({_class[class_number]},{emot[class_number][token]})")
...
def generate_tokens(instruction):
    ...
    four_string_pattern = assembly_pattern1.match(instruction)  # matches   " L1      MOVEM BREG, ='2'"
    three_string_pattern = assembly_pattern2.match(instruction)  # matches     "MOVEM AREG, X"
